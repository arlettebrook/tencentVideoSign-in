...
def get_account_cookie_by_uId(uId):
    for account in config.login_cookie_list.accounts:
        if account.uId == uId:
            return account
    logger.info(f"用户{uId}不存在")
    logger.error("加载cookie配置文件错误")
    exit(-1)


def load_cookie_dict_from_str(uId):
    cookie_dict = {}
    cookie_str = get_account_cookie_by_uId(uId).login_cookie
...
def tencent_video_login(uId):
    account = get_account_cookie_by_uId(uId)
...
def get_cookies(uId):
    login_cookie_dict = load_cookie_dict_from_str(uId)
    login_rsp = tencent_video_login(uId)
    ...
    # 更新cookie
    get_account_cookie_by_uId(uId).login_cookie = json.dumps(auth_cookie)
    return auth_cookie


def tencent_video_auto_sign(uId):
    auth_cookie = get_cookies(uId)
